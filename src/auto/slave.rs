use anyhow::{Context, Result};
use std::time::{Duration, Instant};
use uuid::Uuid;

use crate::auto::dataplane::TestOutcome;
use crate::auto::proto::command::{
    CtrlCommand, Direction, FlowControl, Parity, TestName, TestResultFlag,
};
use crate::auto::proto::parser::{format_command, parse_command};
use crate::port::{open_control, read_crlf_line, retune_for_config, write_line};

#[derive(Debug, Clone)]
pub struct SlaveArgs {
    pub dev: String,
    pub hello_ms: u64,
    pub hello_backoff_max_ms: u64,
    pub repeat_timeout_ms: u64, // for repeat mode end
}

#[derive(Debug, Clone)]
struct CurrentConfig {
    baud: u32,
    parity: Parity,
    bits: u8,
    dir: Direction,
    flow: FlowControl,
}

#[derive(Debug, Default)]
struct TestCtx {
    name: Option<TestName>,
    frames: Option<u64>,
    duration_ms: Option<u64>,
    payload: Option<usize>,
}

pub fn run(args: SlaveArgs) -> Result<()> {
    // Open control channel at 115200 8N1 (line-mode)
    let mut port = open_control(&args.dev)
        .with_context(|| format!("opening control channel on {}", args.dev))?;
    // IDs
    let my_id = Uuid::new_v4().to_string();
    let mut master_id: Option<String> = None;

    eprintln!("[slave] id={} starting HELLO loop", my_id);

    // HELLO loop (sync)
    hello_until_ack_sync(&mut *port, &my_id, args.hello_ms, args.hello_backoff_max_ms)?;

    eprintln!(
        "[slave] id={} got ACK from master id={}, entering main loop",
        my_id,
        master_id.as_deref().unwrap_or("unknown")
    );

    // Main loop state
    let mut cfg: Option<CurrentConfig> = None;
    let mut test = TestCtx::default();

    loop {
        let line = read_crlf_line(&mut *port, None)?;
        if line.is_none() {
            continue;
        }
        if let Ok(cmd) = parse_command(&line.unwrap()) {
            match cmd {
                // Discovery ---------------------------------------------------
                CtrlCommand::Ack { id } => {
                    // Master re-ACKed (rare, but keep the id fresh)
                    master_id = Some(id);
                }
                CtrlCommand::Hello { .. } => {
                    // A master won't send HELLO; ignore stray
                }

                // Config ------------------------------------------------------
                CtrlCommand::ConfigSet {
                    id,
                    baud,
                    parity,
                    bits,
                    dir,
                    flow,
                } => {
                    master_id = Some(id.clone());
                    retune_for_config(&mut *port, baud, parity, bits, flow)
                        .with_context(|| "retuning for CONFIG SET")?;
                    eprintln!(
                        "[slave] config set by {}: baud={} parity={:?} bits={} dir={:?} flow={:?}",
                        id, baud, parity, bits, dir, flow
                    );
                    cfg = Some(CurrentConfig {
                        baud,
                        parity,
                        bits,
                        dir,
                        flow,
                    });

                    // ACK with same fields
                    let ack = CtrlCommand::ConfigSetAck {
                        id: my_id.clone(),
                        baud,
                        parity,
                        bits,
                        dir,
                        flow,
                    };
                    write_line(&mut *port, &format_command(&ack))?;
                }
                CtrlCommand::ConfigSetAck { .. } => {
                    // Slave never expects a ConfigSetAck; ignore
                }

                // Test orchestration -----------------------------------------
                CtrlCommand::TestBegin {
                    id,
                    name,
                    frames,
                    duration_ms,
                    payload,
                } => {
                    test = TestCtx {
                        name: Some(name),
                        frames,
                        duration_ms,
                        payload: Some(payload),
                    };

                    // ACK back
                    let ack = CtrlCommand::TestBeginAck {
                        id: my_id.clone(),
                        name,
                        frames,
                        duration_ms,
                        payload,
                    };
                    write_line(&mut *port, &format_command(&ack))?;

                    // Run the test (blocking) according to config + params
                    if let Some(ref cfg) = cfg {
                        let outcome = run_test_blocking(&args, &mut *port, cfg, &test)?;

                        // End-of-test handshake (repeat mode)
                        end_with_repeat_mode(
                            &mut *port,
                            &my_id,
                            /*ack_responsible=*/ ack_party(cfg.dir),
                            Duration::from_millis(args.repeat_timeout_ms as u64),
                        )?;

                        // Emit TEST RESULT (local)
                        let res = CtrlCommand::TestResult {
                            id: my_id.clone(),
                            result: if outcome.pass {
                                TestResultFlag::Pass
                            } else {
                                TestResultFlag::Fail
                            },
                            rx_frames: outcome.rx_frames,
                            rx_bytes: outcome.rx_bytes,
                            bad_crc: outcome.bad_crc,
                            seq_gaps: outcome.seq_gaps,
                            overruns: outcome.overruns,
                            errors: outcome.errors,
                            rate_bps: outcome.rate_bps,
                            reason: outcome.reason.clone(),
                        };
                        write_line(&mut *port, &format_command(&res))?;

                        // Also print locally (console)
                        if let CtrlCommand::TestResult {
                            result,
                            rx_frames,
                            rx_bytes,
                            bad_crc,
                            seq_gaps,
                            overruns,
                            errors,
                            rate_bps,
                            reason,
                            ..
                        } = &res
                        {
                            eprintln!(
                                "[slave] result={:?} frames={} bytes={} bad_crc={} gaps={} overruns={} errors=0x{:X} rate_bps={} reason={}",
                                result,
                                rx_frames,
                                rx_bytes,
                                bad_crc,
                                seq_gaps,
                                overruns,
                                errors,
                                rate_bps,
                                reason.as_deref().unwrap_or("")
                            );
                        }
                    } else {
                        // No config set yet — fail test immediately
                        let res = CtrlCommand::TestResult {
                            id: my_id.clone(),
                            result: TestResultFlag::Fail,
                            rx_frames: 0,
                            rx_bytes: 0,
                            bad_crc: 0,
                            seq_gaps: 0,
                            overruns: 0,
                            errors: 0,
                            rate_bps: 0,
                            reason: Some("no CONFIG SET before TEST BEGIN".into()),
                        };
                        write_line(&mut *port, &format_command(&res))?;
                    }
                }
                CtrlCommand::TestBeginAck { .. } => {
                    // Slave doesn’t expect this; ignore.
                }

                // End-of-test repeat-mode messages ----------------------------
                CtrlCommand::TestDone { id: _, result: _ } => {
                    // We are the ACK party in HD (RX) or FD (Slave), so ACK
                    let ack = CtrlCommand::TestDoneAck { id: my_id.clone() };
                    write_line(&mut *port, &format_command(&ack))?;
                    // Keep listening; the sender will stop repeating when it sees ACK
                }
                CtrlCommand::TestDoneAck { .. } => {
                    // Not strictly needed on slave (we only send ACKs), ignore
                }

                // Peer RESULT (master’s) --------------------------------------
                CtrlCommand::TestResult { .. } => {
                    // Optional: print/record master’s result
                    // You can parse and mirror to console if you want.
                }

                // Termination -------------------------------------------------
                CtrlCommand::Terminate { .. } => {
                    // Acknowledge and go back to discovery
                    let ack = CtrlCommand::TerminateAck { id: my_id.clone() };
                    write_line(&mut *port, &format_command(&ack))?;
                    // reset local state and start HELLO loop again
                    cfg = None;
                    test = TestCtx::default();
                    hello_until_ack_sync(
                        &mut *port,
                        &my_id,
                        args.hello_ms,
                        args.hello_backoff_max_ms,
                    )?;
                }
                CtrlCommand::TerminateAck { .. } => {
                    // Slave doesn’t expect this; ignore
                }
            }
        }
    }
}

/* -------------------- helpers -------------------- */

fn hello_until_ack_sync(
    port: &mut dyn serialport::SerialPort,
    my_id: &str,
    initial_ms: u64,
    max_ms: u64,
) -> Result<()> {
    let mut backoff = initial_ms.max(50);
    loop {
        let hello = CtrlCommand::Hello {
            id: my_id.to_string(),
        };
        write_line(port, &format_command(&hello))?;

        // poll for ACK within current backoff
        let start = std::time::Instant::now();
        while start.elapsed() < Duration::from_millis(backoff) {
            let line = read_crlf_line(port, None)?;
            if line.is_none() {
                continue;
            }
            if let Ok(cmd) = parse_command(&line.unwrap()) {
                if matches!(cmd, CtrlCommand::Ack { .. }) {
                    return Ok(());
                }
            }
        }
        backoff = (backoff.saturating_mul(2)).min(max_ms.max(initial_ms));
    }
}

/// Which side sends ACK in repeat-mode?
fn ack_party(dir: Direction) -> AckParty {
    match dir {
        Direction::Tx | Direction::Rx => AckParty::WeAckOnRx, // in HD, RX acks
        Direction::Both => AckParty::WeAckAlways,             // as slave in FD, we ack
    }
}

enum AckParty {
    WeAckOnRx,
    WeAckAlways,
}

/// Run the selected test. Calls into your data-plane and returns outcome.
/// This keeps slave.rs small; you implement `run_test_rx_tx` in auto/dataplane.rs.
fn run_test_blocking(
    _args: &SlaveArgs,
    port: &mut dyn serialport::SerialPort,
    cfg: &CurrentConfig,
    test: &TestCtx,
) -> Result<TestOutcome> {
    let name = test.name.ok_or_else(|| anyhow::anyhow!("no test name"))?;
    let frames = test.frames;
    let duration_ms = test.duration_ms;
    let payload = test.payload.unwrap_or(128);

    // Delegate into your dataplane; example signature:
    // run_test_rx_tx(port, cfg.baud, cfg.parity, cfg.bits, cfg.dir, cfg.flow, name, frames, duration_ms, payload)
    // run_test_rx_tx(
    //     port,
    //     cfg.baud,
    //     cfg.parity,
    //     cfg.bits,
    //     cfg.dir,
    //     cfg.flow,
    //     name,
    //     frames,
    //     duration_ms,
    //     payload,
    // )
    Ok(TestOutcome::fail("run_test_blocking not implemented"))
}

/// Implements the end-of-test repeat-mode behavior.
/// - If we are the ACK party, we *only* send ACKs on incoming DONEs.
/// - Otherwise, we send DONE every 500ms until we see an ACK or timeout.
fn end_with_repeat_mode(
    port: &mut dyn serialport::SerialPort,
    my_id: &str,
    ack_party: AckParty,
    timeout: Duration,
) -> Result<()> {
    let start = Instant::now();

    match ack_party {
        AckParty::WeAckOnRx | AckParty::WeAckAlways => {
            // For slave we never originate DONE in your spec; we ACK the other side's DONE.
            // We still need to sit here for the repeat window to catch the DONE and ACK it.
            while start.elapsed() < timeout {
                let line = read_crlf_line(port, Some(timeout))?;
                if line.is_none() {
                    continue;
                }
                if let Ok(cmd) = parse_command(&line.unwrap()) {
                    if let CtrlCommand::TestDone { .. } = cmd {
                        let ack = CtrlCommand::TestDoneAck {
                            id: my_id.to_string(),
                        };
                        write_line(port, &format_command(&ack))?;
                        return Ok(());
                    }
                }
            }
            // Timeout without seeing DONE — fine, proceed
            Ok(())
        }
    }
}
